    ...
    Ts: PredecessorIterable,
{
    ...
    fn edges_from(&self, state: StateIndex<Ts>) -> Option<Self::EdgesFromIter<'_>> {
        Some(self.0.predecessors(state)?.map(Reversed))
    }

    fn state_color(&self, state: StateIndex<Ts>) -> Option<Self::StateColor> {
    ...
    Ts: TransitionSystem + PredecessorIterable,
{
    ...
    fn predecessors(&self, state: StateIndex<Ts>) -> Option<Self::EdgesToIter<'_>> {
