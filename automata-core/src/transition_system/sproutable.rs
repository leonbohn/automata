...
use crate::{innerlude::*, math::Bijection};

use super::{EdgeReference, EdgesFrom, StateIndex};
...
pub trait Sproutable: TransitionSystemBase + Sized {
    ...
    fn sprout_from_ts_with_bijection<Ts>(
        ts: Ts,
    ) -> (Self, Bijection<StateIndex<Ts>, StateIndex<Self>>)
    where
        Self: ForAlphabet<Ts::Alphabet>,
        Ts: TransitionSystemBase<
            ...
            + EdgesFrom,
    {
        ...
        for index in ts.states() {
            map.insert(index.state_index(), out.add_state(index.color().clone()));
        }
        for index in ts.states() {
            let source = *map.get_by_left(&index.state_index()).unwrap();
            for edge in ts.edges_from(index.state_index()).expect("State exists") {
                out.add_edge((
                    source,
...
mod tests {
    ...
    fn sprout_after_creating() {
        let mut ts = DTS::for_alphabet(crate::alphabet!(simple 'a', 'b', 'c'));
        let q0 = ts.add_state(false);
        let q1 = ts.add_state(true);
        assert_eq!(ts.edge(q0, 'a'), None);
        ts.add_edge((q0, 'a', q1));
